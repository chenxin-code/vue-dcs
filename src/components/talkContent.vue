  ...
  <div class="talkWindow" :style="{ height: talkWindowHeight }">
    <div class="talkContent" :class="{ fade: isfade }">
      ...
      >
        ...
        >
          ...
          <div v-if="item.type == 'guess'" class="conversationList">
            <guess-question-list
          ...
          <div v-if="item.type == 'selectuse'" class="conversationList">
            <select-use
        ...
        <div v-show="!store.platformInfo.searchingHistory||recoverTalk" class="welcome">我是您的专属客户经理</div>
        <guess-question-list
          @askguessquestion="askitem"
        ...
        >
          ...
          <!-- <div v-if="item.type=='noanswer'&&item.realcontent.length<1" class="conversationList"><div> 小微被您的问题问住了，要不您换个问法试试，也许会有不一样的收获~ </div></div> -->
          <div
            v-if="item.type == 'relativeanswer' && item.realcontent.length >= 1"
            class="conversationList"
          >
            ...
            <!-- <div v-if="checkAuditTime('8:30','17:30')">
              或者您也可以联系
              <a onclick="onlineChat('3')">人工客服</a>哦
            </div>
            <div v-else>或者您也可以在工作日8:30-17:30咨询人工客服</div> -->
          </div>
          <div v-if="item.type == 'noanswer'" class="conversationList">
            <transfer-to-person></transfer-to-person>
          </div>
          ...
          <div v-if="item.evaluate" class="evaluate_content">
              <div class="like_btn" v-show="item.evaluate!='unlike'">
                <img src="@/assets/img/helpful.png" @click="commitRate(true,item.evaluate,item.id)"  v-show="item.evaluate!='like'"/>
                <img src="@/assets/img/helpful_highline.png"  v-show="item.evaluate=='like'"/>
              </div>
              <div class="unlike_btn" v-show="item.evaluate!='like'">
                <img src="@/assets/img/helpless.png" @click="commitRate(false,item.evaluate,item.id)"  v-show="item.evaluate!='unlike'"/>
    ...
    <div id="inputBar" class="inputBar" :class="{chooseExpression:expressionSelectShow}">
      <div class="changeInput" @click="changeInputType" v-if="!isPc">
        ...
        <img src="@/assets/img/microwphone.png" v-show="inputType == 'typer'&&!(store.platformInfo.feichiez&&getIsWxClient())" />
      </div>
      <voice
      ...
      <div class="inputBackground" v-show="inputType == 'typer'&&!isPc" :style="{height:inputBarBackgroundHeight}"></div>
    </div>
    ...
    <show-frame
      ...
      :usetitle="usetitle"
    ></show-frame>
    <expression-select v-if="expressionSelectShow" @close="hideExpression" @outputemoj="addEmojToInput" @sendContent="sendTyper"></expression-select>
    <bad-rating-cause-form v-if="badRatingCauseShow" :causeList="causeList" @close="closeCauseForm" @commit="recordRateResult"></bad-rating-cause-form>
    <show-tips :tipsText="tipsText" :tipsIcon="tipsIcon" v-if="tipsText"></show-tips>
    <guess-intention v-if="showGuessIntention" :upHeight="selectBarUpHeight" v-show="intentionList.length" :intentionList="intentionList" @commitintention="selectIntention" ></guess-intention>
...
watch(
  ...
  (val) => {
    if (val) {
      ...
      sendtomedia("您好，我是您的数字客户经理小微，请问有什么可以帮您的？");
    ...
    httpszzt.postRobot(requestUrl)
      .then((res) => {
        ...
        if (exchangeButtonAnswer(answer.value)) {
          ...
          addConversation({content: result,styletype: "ansBack",type: "noanswer",realcontent: result,evaluate:evaluate});
        } else if (getXMLNode({ xmlStr: res.data },"Type") == "11") {
          addConversation({content: answer.value,styletype: "ansBack",type: "relativeanswer",realcontent: dealRelativeAnswer(answer.value),evaluate:evaluate}
          );
...
const recordHistory = (str: string) => {
  httpszzt.postRobot(
    ...
    if (getanswer) {
      ...
      addConversation({content: getanswer,styletype: "ansBack",type: "bindToshowMenu",realcontent: false,isSend: "mute"});
    }
  });
};
